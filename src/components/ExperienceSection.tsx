...
const ExperienceSection = () => {
  ...
  const renderIconModal = (iconValue: string) => {
    ...
    return <Briefcase className="h-6 w-6 text-primary" aria-hidden="true" />;
  };
    ...
    <section ref={sectionRef} id="experiencia" className="section-snap bg-muted/30 scroll-mt-16 border-t border-border/30">
      <div className="container mx-auto px-4 py-20 h-full">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="border border-border/40 rounded-xl p-6 bg-card h-full shadow-sm">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {skills.hardSkills.map((skill) => (
                    <Badge key={skill.name} variant="secondary" className="text-xs px-2.5 py-1.5 hover:bg-primary hover:text-primary-foreground transition-colors cursor-default">
                      {skill.name}
                    </Badge>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {skills.softSkills.map((skill) => (
                    <Badge key={skill.name} variant="outline" className="text-xs px-2.5 py-1.5 hover:border-primary hover:text-primary transition-colors cursor-default border-border">
                      {skill.name}
                    </Badge>
                  ))}
                </div>
          ...
          <div className="border border-border/40 rounded-xl p-6 bg-card shadow-sm">
            <div className="flex items-center justify-between mb-8">
                      ...
                      <div className="flex-1 min-w-0">
                        <Dialog>
                          <DialogTrigger asChild>
                            <Card
                              className="cursor-pointer hover:shadow-lg transition-all duration-300 hover:-translate-y-1 hover:border-primary/50 border-border focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none bg-background"
                              role="button"
                              tabIndex={0}
                            ...
                            >
                              <CardHeader className="p-4">
                                <div>
                                  <CardTitle className="text-base md:text-lg font-bold leading-tight mb-1 group-hover:text-primary transition-colors text-foreground">
                                    {exp.role}
                                  </CardTitle>

                                  <div className="mt-1">
                                    <CardDescription className="text-sm font-medium flex items-center gap-1 text-muted-foreground">
                          ...
                          {/* Modal Detalhado Refatorado com Cores Semânticas */}
                          <DialogContent className="sm:max-w-2xl max-h-[85vh] overflow-y-auto bg-background border-border text-foreground gap-0 p-0 overflow-hidden flex flex-col shadow-xl">
                            <DialogHeader className="p-6 pb-4 bg-muted/50 border-b border-border sticky top-0 z-20 backdrop-blur-sm">
                              ...
                              <DialogDescription className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-sm pt-2 text-primary font-medium">
                            ...
                            <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                              ...
                              {exp.achievements && exp.achievements.length > 0 && (
                                <div className="bg-muted/30 p-4 rounded-lg border border-border/50">
                                  <h4 className="font-semibold mb-3 text-foreground text-sm uppercase tracking-wide">Principais Conquistas</h4>
                              ...
                              {exp.technologies && exp.technologies.length > 0 && (
                                <div>
                                  <h4 className="font-semibold mb-3 text-foreground text-sm uppercase tracking-wide">Tech Stack</h4>
