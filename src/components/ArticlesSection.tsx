...
import { Clock, Calendar, ChevronRight, ChevronLeft, Hash } from 'lucide-react';
            ...
            <div className="container mx-auto px-4 py-20 h-full">
                        ...
                        {visibleArticles.map((article, idx) => (
                            ...
                            >
                                <CardHeader>
                                    ...
                                    <CardTitle className="text-xl font-bold line-clamp-2 group-hover:text-primary transition-colors text-foreground">
                                        {article.title}
                                    </CardTitle>
                ...
                {selectedArticle && (
                    <Dialog open={!!selectedArticle} onOpenChange={(open) => !open && setSelectedArticle(null)}>
                        <DialogContent className="sm:max-w-3xl max-h-[85vh] overflow-hidden flex flex-col p-0 gap-0 border-border bg-background shadow-xl">
                            <DialogHeader className="p-6 md:p-8 pb-4 border-b border-border bg-muted/30 sticky top-0 z-10 backdrop-blur-md">
                          ...
                          Removido dark:prose-invert e prose-zinc para respeitar nosso global.css
                        */}
                                <div
                                    className="prose max-w-none 
                            prose-headings:font-bold prose-h3:text-xl prose-h3:mt-6 prose-h3:mb-3 prose-headings:text-foreground
                            prose-p:leading-relaxed prose-p:mb-4 prose-p:text-muted-foreground
                            prose-code:bg-secondary/30 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded-md prose-code:text-sm prose-code:font-mono prose-code:text-foreground"
                                    dangerouslySetInnerHTML={{ __html: selectedArticle.content }}
                                />

                                <div className="mt-8 pt-6 border-t border-border flex flex-wrap gap-2">
                                    {selectedArticle.tags.map(tag => (
                                        <Badge key={tag} variant="secondary" className="text-xs bg-secondary/50 text-foreground">
